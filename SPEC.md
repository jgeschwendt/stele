...
Draft 0.10 · 2026-09-17 · status: refined — gigarefine converged at cap (5 passes, curve 33→21→11→5→3-trivial); 0.8 adds the `.steleignore` scan-exclusion decision from v1 self-hosting; 0.9 adds undercover mode — a private single-user graph for shared repos whose collaborators never see a stele artifact; 0.10 replaces the `stele:` sigil with the glyph notation (`※` `⊨` `§`, `@stele`/`@end`), lock `version` 2, engine 0.3.0, migrated by `stele migrate`
...
invariants: # claims with anchors; see 2.4
  - claim: memories are append-only; supersede, never mutate
    anchor: ※ memory-supersede # comment anchor (2.5) or path#symbol
...
- Every claim carries an anchor (a `※` landmark token or `path#symbol`). An unanchored claim is rejected at build — provenance or it doesn't compile (hearsay gate).
...
- A claim is addressed **`<node-id>/<slug>`**, where `<slug>` is derived from the claim's anchor: for a `※ <slug>` landmark anchor, the landmark slug verbatim; for a `<path>#<symbol>` anchor, `<symbol>` lowercased with each maximal run of non-`[a-z0-9]` characters collapsed to a single `-` and leading/trailing `-` stripped (so `refund.ex#changeset` → `changeset`, `money.ts#MoneyType` → `moneytype`). Two claims in one node whose derived slugs are equal → exit 2 (naming both). This derived slug is the claim's lock `id` (§3.2) and the address the `⊨` comment (§2.5) and `stele blame` take — the comment carries it as a glyph token, `blame` takes it bare (§5.1). The node-id may be abbreviated to its final path segment when unambiguous across the graph, so `apps/web/lib/billing/refund-cap` and `billing/refund-cap` name the same claim.
...
**Two anchor namespaces**, both valid in `anchor:`: (1) `※ <slug>` — a landmark; resolves to the unique `// ※ <slug>` comment. The token shape is fixed — glyph, one ASCII space, slug (§2.5) — and `<slug>` matches the comment slug verbatim, byte for byte, so the field quotes the comment exactly; cardinality-1 is enforced in §4.1. A malformed slug in this FIELD → exit 2 (the field is authored schema, not scanned prose; §2.5). (2) `<path>#<symbol>` — a direct binding; `<path>` is a repo-root-relative file, `<symbol>` resolves via tree-sitter to a named definition (function/module/class/etc.) in that file; in markdown (parser-less) it resolves against ATX heading slugs — exact slug match or slug-prefix at a hyphen boundary (`c1` matches `## C1: …`, never `## C10: …`) — with `digest: null` (§4.5 churn fallback). Cardinality MUST be 1 (0 → unresolved, >1 → ambiguous; both exit 1). No comment required — survives edits inside the symbol but breaks on rename/move (the EXAMPLE §4 trade-off).

**Scan scope** for referential + cardinality checks (§4.1): VCS-tracked files only (respects `.gitignore`), excluding `.stele/` and any path matched by a committed root **`.steleignore`** (gitignore syntax). A `.steleignore`d path is invisible to ALL build/check source scanning alike — node discovery (its AGENTS.md declares nothing, §3.1), the anchor scan, and import extraction — so vendored fixture trees and docs that quote anchor syntax cannot pollute the graph. `.steleignore` is a source artifact, not configuration: it is an input to `build` and therefore to the lock, unlike `.stele/config.toml` (§3.4), which stays check/emit-time only. AGENTS.md files ARE scanned so `⊨` claim back-references resolve; `※` and `⊨` are distinct tokens and never collide. `resolved` (`file:line`) is recomputed every build and stored in the lock. In undercover mode (§3.5) node discovery instead walks the overlay under `<home>/.stele/tree/` (a plain filesystem walk, never `git ls-files` and never `.steleignore`-filtered); every other scan named here — anchors, extraction, exhaustiveness, freshness — stays VCS-tracked-only.

### 2.5 Comment anchor grammar

Language-native comments, a glyph token, **identity and binding only — never description** (descriptive anchors restate code and rot [C17, C25]):

```
// ※ <slug>                   — declares a landmark; slug-match cardinality MUST be 1
// ⊨ <node-id>/<slug>         — binds this region to a declared claim
```

**Token shape, everywhere.** A glyph token is the glyph, ONE ASCII space, the payload: `※ refund-cap`, `⊨ billing/refund-cap`, `§ 0007` (§2.6). The same bytes in a comment, in `anchor:` (§2.2), and in `decided_by:` — a node file quotes the code verbatim, and one `rg '※ '` finds declaration and reference alike. In comments the payload ends at the first whitespace or end of line; no other separator, no trailing punctuation. `✻` is reserved for the human-notes convention and is never a stele token (§10 item 2).

`<slug>` lexeme: `[a-z0-9]+(-[a-z0-9]+)*` (lowercase alphanumerics, single hyphens, no leading/trailing/doubled hyphen), terminated by the first whitespace or end-of-line after `※ `. Cardinality (§4.1) counts comments whose slug is byte-for-byte EQUAL to the queried slug — not a word-bounded substring: `※ refund-cap` and `※ refund-cap-2` are distinct landmarks and never inflate each other's count (hyphens are word-boundary characters, so a substring rule would false-match).

**Silent non-match.** A `※` or `⊨` whose following text is not a well-formed slug (landmark) or `<node-id>/<slug>` address (claim) is prose: it is ignored, never an error. `※` is an everyday annotation mark in CJK comments, so a non-matching occurrence MUST be silent — a comment carries no exit-2 condition at all. The `anchor:` FIELD keeps exit 2 on a malformed slug (§2.4): it is authored schema, not scanned prose. A typo'd landmark that a node file references therefore fails referentially rather than lexically — slug-match cardinality 0 → exit 1 (§4.1).
...
Two forms, no more. `※` gives claims and queries a stable, move-proof address (renames don't break anchors; the registry does — and CI catches it). `⊨` closes the loop from a declared invariant/hazard to the code region it governs — it reads "this region satisfies that claim", leaving `⊢` (proves) to what `enforced_by` tests do. Prior art: todocheck proves comment-claims-validated-in-CI ships; AIDEV-NOTE proves agents grep for anchor prefixes; nothing combines them [C25].
...
A `decided_by` entry is the token `§ <NNNN>` — glyph, one ASCII space, the record's number as the filename zero-pads it (`§ 0007`) — resolved against the detected ADR directory (`adr/`, `doc/adr/`, `docs/adr/`; §10 item 5). ADR filenames are unchanged. The lock keeps the path-derived id `<adrdir>/<NNNN>` (§3.2), so `doc/adr` and `docs/adr` repos need no disambiguation in the authored token.
...
2. **The generated region** — exactly ONE marker-fenced region per file, opened by `<!-- @stele -->` and closed by `<!-- @end -->`, rendered by `emit`. `@stele` takes an OPTIONAL region name: the first token after `@stele `, defaulting to `router` when absent (`router` is the only name v1 emits); any text after the name and before `-->` is free annotation the parser ignores (so the root's `@stele router · generated … · do not hand-edit`, an interior node's bare `@stele router`, and a bare `@stele` are equivalent). `@end` is bare — it carries no name. All bytes strictly between the markers are engine-owned; the markers and everything outside them are preserved verbatim. Regions do not nest. A `@stele` with no matching `@end`, an `@end` with no `@stele`, two `@stele` markers, or nesting → exit 2 (naming the file). A node's AGENTS.md that has a `stele` block but no generated region → `emit` exits 2 (`run stele init` to scaffold it); `emit` only rewrites between existing markers and never creates the region itself. Contents vary by node kind: the root renders the hazard banner (top-N active hazards across the graph), the router/index of child + depended nodes (id, kind, purpose, unfold pointer), the transpose-index pointers (§6.1), and the engine instructions; interior nodes render their resolved-anchor table (anchor → file:line) plus a child router where they have children. Identity and commands are never generated — they live in the authored block (§6 items 1–2). `emit --check` fails CI when a generated region diverges from the graph. Rendering is deterministic (stable node/edge ordering): the same graph always emits byte-identical regions — the property `emit --check`'s byte-diff relies on, and the mitigation that keeps generated-region merge conflicts rare and mechanical.
...
**Top-level:** `{ "version": 2, "nodes": {"<id>": <node>}, "adrs": {"<id>": {"number":7,"status":"accepted","path":"adr/0007-integer-cents.md"}}, "landmarks": {"<slug>": {"file":"…","line":18,"node":"<id>"}} }`. Unknown `version` → reject (exit 2), never best-effort parse. The lock is unchanged in SHAPE: ADR ids stay path-derived (`adr/0007`, the resolved form of an authored `§ 0007`, §2.6) and `landmarks{}` stays keyed by the bare slug — the glyph lives in the authored token, never in a lock key. The one place it appears is a claim's `anchor`, a verbatim copy of what was authored: `※ refund-cap`, or `money.ts#Money` for a direct binding.

**Version 2 is the glyph notation** (§2.5). A `"version": 1` lock is the pre-0.3.0 `stele:` grammar: `check`/`emit` reject it as an input error (exit 2) with the fix line `run stele migrate, then stele build` (§5.1). There is no dual-read — the engine parses one grammar.
...
- `CLAUDE.md` = `@AGENTS.md` (one line), written at the repo root AND beside every nested node's `AGENTS.md` — only-if-absent at each (an existing file is the team's; `emit --check` never diffs a shim). Verified empirically 2026-09-16 (2.1.273): no native AGENTS.md loading at any level; a nested shim is lazily loaded on first read under its directory, and pulls its sibling `AGENTS.md` with it. Negative claim — re-verify [C2].
...
**The one materialized file.** Undercover renders exactly one file into the work tree: `CLAUDE.local.md` at the invoking work-tree root, containing a single relative `@`-import of the overlay root node file (normal checkout: `@.stele/tree/AGENTS.md`; a worktree one level below the home: `@../.stele/tree/AGENTS.md`). §3.3's nested `CLAUDE.md` shims are normal-mode only: nothing is written beside an overlay node under `.stele/tree/`, nor anywhere in the work tree but this shim. Claude Code auto-loads `CLAUDE.local.md` alongside `CLAUDE.md`, and its docs recommend gitignoring it (verified against live docs 2026-07-21) — the shim is the private analogue of the §3.3 `CLAUDE.md` = `@AGENTS.md` line. To keep the work tree clean, a marker-fenced managed block in `<common-dir>/info/exclude` — the lines `# stele:begin undercover` … `# stele:end undercover`, rewritten in place idempotently and never touching a line outside the fence (an internal managed block in a git file, NOT stele notation — §2.5's grammar does not reach it, and it is unchanged by the glyph cut, so existing installs self-heal) — unconditionally carries `/CLAUDE.local.md` and `/.stele/`. Excess entries are harmless, and the block lives in the shared common dir, so one block covers every worktree. The block is installed **before any artifact is written** (`init --undercover` writes the exclude first, so a failed exclude write aborts having created nothing — no window in which `.stele/` is visible) and its write is atomic (temp-file + rename); every undercover `build`/`check`/`emit` re-ensures it (a no-op when unchanged), so a lost or partial block self-heals on the next run and no artifact ever surfaces in `git status`. The shim is written at the invoking work-tree root resolved via `git rev-parse --show-toplevel`, not the cwd, so a subdirectory invocation still lands it where the root-anchored `/CLAUDE.local.md` entry matches.

**Mutual exclusion.** An undercover marker coexisting with a tracked `stele`-block `AGENTS.md` or a tracked `.stele/` in the work tree → exit 2: a repo already carrying a shared, committed graph cannot also run undercover. Mixing the two is a v2 concern. A submodule checkout is likewise refused → exit 2: its git common dir is `<superproject>/.git/modules/<name>`, so the graph home would fall inside the superproject's `.git` directory (a `.git` component in the resolved home is the signal). Also a v2 concern.
...
Every anchor resolves (file exists, symbol found via tree-sitter); every `decided_by` names an existing, non-superseded ADR; every `enforced_by` artifact exists; every landmark has slug-match cardinality exactly 1 (comments whose slug is byte-for-byte equal, not a word-bounded substring; §2.5); every `⊨ <node-id>/<slug>` comment resolves to a declared claim (a dangling `⊨` binding, or one naming a non-existent node/slug, fails referential; a `⊨` whose payload is not an address at all is prose and is never counted, §2.5).
...
stele migrate                             # rewrite the pre-0.3.0 `stele:` grammar to glyph tokens in place (§2.5)
stele build | init                        # all read/write commands take --json
```

**Addresses on the command line are ASCII.** `stele blame billing/refund-cap`, `stele node billing`, `stele unfold apps/web` — no glyph is ever typed into a shell; the glyph belongs to the authored token, and output prints the glyph forms (`※ refund-cap`, `§ 0007`).
...
**Exit codes** (uniform across subcommands): `0` success (`check` clean); `1` assertion failure (≥1 violation from `check`/`emit --check` — reserved for "repo out of spec", never tool malfunction); `2` input error (malformed `stele` block: YAML parse error, unknown field, wrong type; malformed `anchor:` slug, §2.5; duplicate/colliding node id; unknown or superseded lock `version` — a `version: 1` lock's fix line is `run stele migrate, then stele build`, §3.2; committed lock ≠ freshly-built graph; bad flags); `3` internal error (tree-sitter/extractor crash, IO).

**Build atomicity:** any exit-2 condition prints offending `file:line` and aborts; `build` writes NO lock (never a partial graph). `query`/`serve`/`emit` read the committed lock and never rebuild — a missing lock → exit 2 "run stele build"; they trust the committed lock's freshness, since detecting staleness is `check`'s job, not theirs. They exit 2 only on a missing or unknown-`version` lock, never on staleness. `check` also requires a committed lock but rebuilds in-memory and byte-compares its canonical serialization to it (§5.1); committed lock ≠ freshly-built graph → exit 2 "run stele build". None of the four ever writes a lock (only `build` does; `init` writes source blocks, never the lock). `migrate` writes source files only and requires no lock: it exits `0` whether or not it rewrote anything (a no-op run is success, since it is idempotent), `2` on an input error (bad flags, an unreadable or unparseable source file — nothing is written, as for `build`), `3` on IO failure; it never exits `1`, asserting nothing. Throughout this contract *the committed lock* names the committed lock in normal mode and the private on-disk lock at the graph home in undercover mode (§3.5); the byte-compare and every exit-2 condition above are mode-independent.
...
- `stele migrate` — the upgrade path, not the greenfield one: a repo already carrying the pre-0.3.0 `stele:` notation rewrites in place (§5.1), then `stele build` re-locks it at `version` 2 (§3.2). A repo scaffolded by `stele init` at 0.3.0 or later never runs it.
...
2. **Anchor sigil: `stele:`** — greppable, self-naming, deliberately distinct from the `✻` human-notes convention. _Superseded by the glyph notation (decision log, 2026-09-17): the sigil is greppable but collides with every doc, test and comment that merely quotes it; the glyphs keep the grep surface without a fence. `✻` stays reserved for human notes._
...
- **Glyph notation: `※` landmark, `⊨` claim, `§` decision, `@stele`/`@end` region** (0.3.0) — the `stele:` sigil was greppable at the cost of colliding with every file that merely mentions it: docs quoting the grammar, tests asserting on it, the fixture repo vendoring a whole stele tree (the `.steleignore` patch of 0.8 was that collision, once). A glyph is zero-collision without a bracket fence, which is the alternative the fence would have cost the grep surface. `※` (REFERENCE MARK, U+203B) renders in every CJK-capable font and already means "see this"; `⊨` (TRUE, U+22A8) reads "the region satisfies the claim", leaving `⊢` (proves) for what `enforced_by` tests do; `§` is the printer's mark for a numbered record, so `§ 0007` is an ADR reference on sight; `✻` stays reserved for human notes and never enters the grammar (§10 item 2, now superseded). The region marker stays ASCII — `<!-- @stele -->` / `<!-- @end -->` — because it is the most machine-handled token in the system and the least read. Glyph, one ASCII space, payload EVERYWHERE (§2.5), so the `anchor:` field quotes the comment verbatim and one grep finds both sides. A `※` whose payload is not a slug is prose and is silently ignored: the mark is ordinary annotation in CJK comments, so lexical strictness there would fail honest files — a typo'd landmark still fails referentially, at cardinality 0 (§4.1). Hard cut over dual-read: lock `version` 1 → 2, a v1 lock exits 2 pointing at `stele migrate` (§3.2), and `migrate` (§5.1) is the one-shot bridge — carrying two grammars in the scanner is permanent cost for a one-time move. Cost accepted: `※` and `§` are East-Asian-Width *ambiguous* (two cells in a CJK-locale terminal, one elsewhere), so an anchor table can lose column alignment there; `⊨` is neutral and `@stele` is ASCII [user decision 2026-09-17].
